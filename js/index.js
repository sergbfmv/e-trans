...
function createModal(title, description) {
  ...
  const descriptionElem = createElem('p', { className: 'modal__description', textContent: description })
  const formElem = createElem('form', { className: 'modal__form', method: 'post', action: 'https://jsonplaceholder.typicode.com/posts', id: 'order' })
  const nameLabelElem = createElem('label', { className: 'modal__label' })
  const nameSpanElem = createElem('span', { className: 'modal__span', textContent: 'Имя' })
  const nameInputElem = createElem('input', { className: 'modal__input', placeholder: 'Введите Ваше имя', name: 'name', required: true })

  const phoneLabelElem = createElem('label', { className: 'modal__label' })
  const phoneSpanElem = createElem('span', { className: 'modal__span', textContent: 'Телефон' })
  const phoneInputElem = createElem('input', { className: 'modal__input', placeholder: 'Введите Ваш телефон', name: 'phone', required: true })
  ...
  const closeModalBtn = createElem('button', {
    className: 'modal__close', innerHTML: `
